...
func (nc *NodeConfig) GetViperConfig(v *viper.Viper) error {
	...
	nc.DataAvailability.StartHeight = v.GetUint64(FlagDAStartHeight)
	nc.DataAvailability.BlockTime = v.GetDuration(FlagDABlockTime)
...
func AddFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Duration(FlagDABlockTime, def.DataAvailability.BlockTime, "DA chain block time (for syncing)")
	...
	cmd.Flags().Uint64(FlagDAStartHeight, def.DataAvailability.StartHeight, "starting DA block height (for syncing)")
