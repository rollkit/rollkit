...
// 			_, err := dalcKV.Get(context.Background(), ds.NewKey(string(tt.key)))
...
// 	for _, tt := range tc {
// 		t.Run(tt.name, func(t *testing.T) {
// 			err := dalcKV.Put(context.Background(), ds.NewKey(string(tt.key)), tt.value)
...
// 	for _, tt := range tc {
// 		t.Run(tt.name, func(t *testing.T) {
// 			err := dalcKV.Delete(context.Background(), ds.NewKey(string(tt.key)))
// 			if !errors.Is(err, tt.err) {
// 				t.Errorf("Invalid err, got: %v expected %v", err, tt.err)
// 			}
// 		})
