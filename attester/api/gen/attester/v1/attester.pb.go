// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.35.2
// 	protoc        v5.29.3
// source: attester/v1/attester.proto

package attesterv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// SubmitBlockRequest contains the details of the block to be attested.
type SubmitBlockRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	BlockHeight uint64 `protobuf:"varint,1,opt,name=block_height,json=blockHeight,proto3" json:"block_height,omitempty"` // The height of the block.
	BlockHash   []byte `protobuf:"bytes,2,opt,name=block_hash,json=blockHash,proto3" json:"block_hash,omitempty"`        // The hash of the block (e.g., 32 bytes).
	DataToSign  []byte `protobuf:"bytes,3,opt,name=data_to_sign,json=dataToSign,proto3" json:"data_to_sign,omitempty"`   // The specific data payload that needs to be signed by the attesters.
}

func (x *SubmitBlockRequest) Reset() {
	*x = SubmitBlockRequest{}
	mi := &file_attester_v1_attester_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SubmitBlockRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SubmitBlockRequest) ProtoMessage() {}

func (x *SubmitBlockRequest) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SubmitBlockRequest.ProtoReflect.Descriptor instead.
func (*SubmitBlockRequest) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{0}
}

func (x *SubmitBlockRequest) GetBlockHeight() uint64 {
	if x != nil {
		return x.BlockHeight
	}
	return 0
}

func (x *SubmitBlockRequest) GetBlockHash() []byte {
	if x != nil {
		return x.BlockHash
	}
	return nil
}

func (x *SubmitBlockRequest) GetDataToSign() []byte {
	if x != nil {
		return x.DataToSign
	}
	return nil
}

// SubmitBlockResponse indicates the result of the submission attempt.
type SubmitBlockResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Accepted     bool   `protobuf:"varint,1,opt,name=accepted,proto3" json:"accepted,omitempty"`                            // True if the node accepted the request (e.g., it's the leader).
	ErrorMessage string `protobuf:"bytes,2,opt,name=error_message,json=errorMessage,proto3" json:"error_message,omitempty"` // Contains an error message if accepted is false.
	LeaderHint   string `protobuf:"bytes,3,opt,name=leader_hint,json=leaderHint,proto3" json:"leader_hint,omitempty"`       // Provides the address of the current leader if known and this node is not the leader.
}

func (x *SubmitBlockResponse) Reset() {
	*x = SubmitBlockResponse{}
	mi := &file_attester_v1_attester_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SubmitBlockResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SubmitBlockResponse) ProtoMessage() {}

func (x *SubmitBlockResponse) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SubmitBlockResponse.ProtoReflect.Descriptor instead.
func (*SubmitBlockResponse) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{1}
}

func (x *SubmitBlockResponse) GetAccepted() bool {
	if x != nil {
		return x.Accepted
	}
	return false
}

func (x *SubmitBlockResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

func (x *SubmitBlockResponse) GetLeaderHint() string {
	if x != nil {
		return x.LeaderHint
	}
	return ""
}

// SubmitSignatureRequest contains a signature from a specific attester for a block.
type SubmitSignatureRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	BlockHeight uint64 `protobuf:"varint,1,opt,name=block_height,json=blockHeight,proto3" json:"block_height,omitempty"` // The height of the block being signed.
	BlockHash   []byte `protobuf:"bytes,2,opt,name=block_hash,json=blockHash,proto3" json:"block_hash,omitempty"`        // The hash of the block being signed.
	AttesterId  string `protobuf:"bytes,3,opt,name=attester_id,json=attesterId,proto3" json:"attester_id,omitempty"`     // The unique ID of the attester node submitting the signature.
	Signature   []byte `protobuf:"bytes,4,opt,name=signature,proto3" json:"signature,omitempty"`                         // The attester's signature over the data_to_sign for this block.
}

func (x *SubmitSignatureRequest) Reset() {
	*x = SubmitSignatureRequest{}
	mi := &file_attester_v1_attester_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SubmitSignatureRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SubmitSignatureRequest) ProtoMessage() {}

func (x *SubmitSignatureRequest) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SubmitSignatureRequest.ProtoReflect.Descriptor instead.
func (*SubmitSignatureRequest) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{2}
}

func (x *SubmitSignatureRequest) GetBlockHeight() uint64 {
	if x != nil {
		return x.BlockHeight
	}
	return 0
}

func (x *SubmitSignatureRequest) GetBlockHash() []byte {
	if x != nil {
		return x.BlockHash
	}
	return nil
}

func (x *SubmitSignatureRequest) GetAttesterId() string {
	if x != nil {
		return x.AttesterId
	}
	return ""
}

func (x *SubmitSignatureRequest) GetSignature() []byte {
	if x != nil {
		return x.Signature
	}
	return nil
}

// SubmitSignatureResponse indicates the result of the signature submission.
type SubmitSignatureResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Success bool `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"` // True if the signature was received and initially processed.
	// Final aggregation status is handled asynchronously.
	ErrorMessage string `protobuf:"bytes,2,opt,name=error_message,json=errorMessage,proto3" json:"error_message,omitempty"` // Optional error message if processing failed immediately.
}

func (x *SubmitSignatureResponse) Reset() {
	*x = SubmitSignatureResponse{}
	mi := &file_attester_v1_attester_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SubmitSignatureResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SubmitSignatureResponse) ProtoMessage() {}

func (x *SubmitSignatureResponse) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SubmitSignatureResponse.ProtoReflect.Descriptor instead.
func (*SubmitSignatureResponse) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{3}
}

func (x *SubmitSignatureResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *SubmitSignatureResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

// GetAggregatedSignatureRequest requests the aggregated signatures for a block.
type GetAggregatedSignatureRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	BlockHeight uint64 `protobuf:"varint,1,opt,name=block_height,json=blockHeight,proto3" json:"block_height,omitempty"` // The height of the block to query.
}

func (x *GetAggregatedSignatureRequest) Reset() {
	*x = GetAggregatedSignatureRequest{}
	mi := &file_attester_v1_attester_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAggregatedSignatureRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAggregatedSignatureRequest) ProtoMessage() {}

func (x *GetAggregatedSignatureRequest) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAggregatedSignatureRequest.ProtoReflect.Descriptor instead.
func (*GetAggregatedSignatureRequest) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{4}
}

func (x *GetAggregatedSignatureRequest) GetBlockHeight() uint64 {
	if x != nil {
		return x.BlockHeight
	}
	return 0
}

// GetAggregatedSignatureResponse returns the aggregated signatures if quorum was met.
type GetAggregatedSignatureResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	QuorumMet  bool     `protobuf:"varint,1,opt,name=quorum_met,json=quorumMet,proto3" json:"quorum_met,omitempty"` // True if the quorum threshold was met for this block height.
	Signatures [][]byte `protobuf:"bytes,2,rep,name=signatures,proto3" json:"signatures,omitempty"`                 // The list of collected signatures if quorum was met.
	// Empty if quorum was not met or the height is unknown.
	ErrorMessage string `protobuf:"bytes,3,opt,name=error_message,json=errorMessage,proto3" json:"error_message,omitempty"` // Optional error message if the query failed.
}

func (x *GetAggregatedSignatureResponse) Reset() {
	*x = GetAggregatedSignatureResponse{}
	mi := &file_attester_v1_attester_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAggregatedSignatureResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAggregatedSignatureResponse) ProtoMessage() {}

func (x *GetAggregatedSignatureResponse) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAggregatedSignatureResponse.ProtoReflect.Descriptor instead.
func (*GetAggregatedSignatureResponse) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{5}
}

func (x *GetAggregatedSignatureResponse) GetQuorumMet() bool {
	if x != nil {
		return x.QuorumMet
	}
	return false
}

func (x *GetAggregatedSignatureResponse) GetSignatures() [][]byte {
	if x != nil {
		return x.Signatures
	}
	return nil
}

func (x *GetAggregatedSignatureResponse) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
}

// BlockInfo represents the attested information for a block, stored in the FSM state.
type BlockInfo struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Height     uint64 `protobuf:"varint,1,opt,name=height,proto3" json:"height,omitempty"`
	Hash       []byte `protobuf:"bytes,2,opt,name=hash,proto3" json:"hash,omitempty"`                                 // Block hash (must match state.BlockHashSize).
	DataToSign []byte `protobuf:"bytes,3,opt,name=data_to_sign,json=dataToSign,proto3" json:"data_to_sign,omitempty"` // The data that was actually signed.
}

func (x *BlockInfo) Reset() {
	*x = BlockInfo{}
	mi := &file_attester_v1_attester_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BlockInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BlockInfo) ProtoMessage() {}

func (x *BlockInfo) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BlockInfo.ProtoReflect.Descriptor instead.
func (*BlockInfo) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{6}
}

func (x *BlockInfo) GetHeight() uint64 {
	if x != nil {
		return x.Height
	}
	return 0
}

func (x *BlockInfo) GetHash() []byte {
	if x != nil {
		return x.Hash
	}
	return nil
}

func (x *BlockInfo) GetDataToSign() []byte {
	if x != nil {
		return x.DataToSign
	}
	return nil
}

// FSMState represents the serializable state of the AttesterFSM for snapshots.
type FSMState struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Blocks []*BlockInfo `protobuf:"bytes,1,rep,name=blocks,proto3" json:"blocks,omitempty"` // List of all attested block information.
}

func (x *FSMState) Reset() {
	*x = FSMState{}
	mi := &file_attester_v1_attester_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FSMState) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FSMState) ProtoMessage() {}

func (x *FSMState) ProtoReflect() protoreflect.Message {
	mi := &file_attester_v1_attester_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FSMState.ProtoReflect.Descriptor instead.
func (*FSMState) Descriptor() ([]byte, []int) {
	return file_attester_v1_attester_proto_rawDescGZIP(), []int{7}
}

func (x *FSMState) GetBlocks() []*BlockInfo {
	if x != nil {
		return x.Blocks
	}
	return nil
}

var File_attester_v1_attester_proto protoreflect.FileDescriptor

var file_attester_v1_attester_proto_rawDesc = []byte{
	,
}

var (
	file_attester_v1_attester_proto_rawDescOnce sync.Once
	file_attester_v1_attester_proto_rawDescData = file_attester_v1_attester_proto_rawDesc
)

func file_attester_v1_attester_proto_rawDescGZIP() []byte {
	file_attester_v1_attester_proto_rawDescOnce.Do(func() {
		file_attester_v1_attester_proto_rawDescData = protoimpl.X.CompressGZIP(file_attester_v1_attester_proto_rawDescData)
	})
	return file_attester_v1_attester_proto_rawDescData
}

var file_attester_v1_attester_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_attester_v1_attester_proto_goTypes = []any{
	(*SubmitBlockRequest)(nil),             // 0: attester.v1.SubmitBlockRequest
	(*SubmitBlockResponse)(nil),            // 1: attester.v1.SubmitBlockResponse
	(*SubmitSignatureRequest)(nil),         // 2: attester.v1.SubmitSignatureRequest
	(*SubmitSignatureResponse)(nil),        // 3: attester.v1.SubmitSignatureResponse
	(*GetAggregatedSignatureRequest)(nil),  // 4: attester.v1.GetAggregatedSignatureRequest
	(*GetAggregatedSignatureResponse)(nil), // 5: attester.v1.GetAggregatedSignatureResponse
	(*BlockInfo)(nil),                      // 6: attester.v1.BlockInfo
	(*FSMState)(nil),                       // 7: attester.v1.FSMState
}
var file_attester_v1_attester_proto_depIdxs = []int32{
	6, // 0: attester.v1.FSMState.blocks:type_name -> attester.v1.BlockInfo
	0, // 1: attester.v1.AttesterService.SubmitBlock:input_type -> attester.v1.SubmitBlockRequest
	2, // 2: attester.v1.AttesterService.SubmitSignature:input_type -> attester.v1.SubmitSignatureRequest
	4, // 3: attester.v1.AttesterService.GetAggregatedSignature:input_type -> attester.v1.GetAggregatedSignatureRequest
	1, // 4: attester.v1.AttesterService.SubmitBlock:output_type -> attester.v1.SubmitBlockResponse
	3, // 5: attester.v1.AttesterService.SubmitSignature:output_type -> attester.v1.SubmitSignatureResponse
	5, // 6: attester.v1.AttesterService.GetAggregatedSignature:output_type -> attester.v1.GetAggregatedSignatureResponse
	4, // [4:7] is the sub-list for method output_type
	1, // [1:4] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_attester_v1_attester_proto_init() }
func file_attester_v1_attester_proto_init() {
	if File_attester_v1_attester_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_attester_v1_attester_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_attester_v1_attester_proto_goTypes,
		DependencyIndexes: file_attester_v1_attester_proto_depIdxs,
		MessageInfos:      file_attester_v1_attester_proto_msgTypes,
	}.Build()
	File_attester_v1_attester_proto = out.File
	file_attester_v1_attester_proto_rawDesc = nil
	file_attester_v1_attester_proto_goTypes = nil
	file_attester_v1_attester_proto_depIdxs = nil
}
