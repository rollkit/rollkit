...
func getRPC(t *testing.T) (*mocks.Application, rpcclient.Client) {
	...
	n, err := node.NewNode(context.Background(), config.NodeConfig{DAAddress: mocks.MockDAAddress, DANamespace: mocks.MockDANamespace, Aggregator: true, BlockManagerConfig: config.BlockManagerConfig{BlockTime: 1 * time.Second}, Light: false}, key, signingKey, proxy.NewLocalClientCreator(app), &cmtypes.GenesisDoc{ChainID: "test", Validators: genesisValidators}, node.DefaultMetricsProvider(cmconfig.DefaultInstrumentationConfig()), log.TestingLogger())
	require.NoError(err)
	require.NotNil(n)
