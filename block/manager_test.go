...
func TestTrySyncNextBlockMultiple(t *testing.T) {
	...
	err := fullStore.UpdateState(sampleState)
	require.NoError(t, err)
	fullStore.SetHeight(sampleState.LastBlockHeight)
	...
	cases := []struct {
		...
		{
			name:     "try_sync_next_block_multiple",
			store:    fullStore,
			genesis:  genesis,
			DAHeight: 123,
		},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			logger := test.NewFileLoggerCustom(t, test.TempLogFileName(t, c.name))
			dalc := &da.DAClient{DA: goDATest.NewDummyDA(), Logger: logger}
			manager, err := NewManager(key, conf, c.genesis, c.store, nil, nil, dalc, nil, logger, nil)
			assert.NoError(err)
			...
			manager.blockCache.setBlock(height+1, block)
			err = manager.trySyncNextBlock(ctx, state.DAHeight)
			require.NoError(t, err)
		})
	}
}
